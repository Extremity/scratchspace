...
## Research Phase 6: Implementation Plan (COMPLETE)

**Documents:** `plans/dflash-solutions/` (5 files: task6-*)

### Question

> Produce an exact, source-grounded implementation plan for adding opt-in minimal-replay DFlash via `--beefix-dflash-custom` — verifying all Task 4-5 claims against current source.

### Approach

5 verification subtasks confirmed research claims against actual source code, followed by final blueprint compilation.

### Key Findings

**Source verification (all claims VERIFIED, zero discrepancies):**
- `need_n_rs_seq()` at [`common/common.h:417`](common/common.h:417) includes DFlash — VERIFIED
- RS allocation `n_rows = mem_size * (1 + n_rs_seq)` at [`llama-memory-recurrent.cpp:99`](src/llama-memory-recurrent.cpp:99) — VERIFIED
- Opt-in boundary at [`common/common.cpp:1770`](common/common.cpp:1770) — cleanest single control point
- R/S state tensors, cell indexing, `s_copy()` (computes row indices, NOT data copy) — VERIFIED
- `seq_cp()` copies metadata only — VERIFIED
- Verification lifecycle at [`server-context.cpp:4213-4274`](tools/server/server-context.cpp:4213) — VERIFIED
- Tape capture tensors (`k_in`, `v_in`, `g_in`, `b_in`) at [`delta-net-base.cpp:49-53`](src/models/delta-net-base.cpp:49) — VERIFIED
- GDN replay via `ggml_gated_delta_net` (existing primitive, no new CUDA kernel) — VERIFIED

**CRITICAL VRAM DISCREPANCY FOUND:**
- Task 5 estimated tape buffer at ~24 MB. Actual: ~6,552 MiB because v/g/b tensors have S_v=786,432 dimensions (not just k with n_embd_r=30,720).
- Naive GPU tape WORSENS VRAM (9.1 GiB vs 6.2 GiB upstream).
- **CPU tape is the only viable strategy:** saves ~3.5 GB GPU VRAM at ~26 ms transfer overhead per cycle.

**Corrected VRAM accounting:**
- Current upstream: ~6,187 MiB GPU overhead (RS snapshots)
- Custom mode: ~2,598 MiB GPU (backup cells + base state) + ~6,552 MiB CPU (tape) = 58% GPU VRAM savings
- Backup cells: ~150 MiB per cell (S state dominates)

**Fallback: 7 failure conditions** all fall back to checkpoint rollback (always available, created before speculation).

**Implementation scope:** 3 new files (~670 lines), 10 modified files (~236 lines changed), 6 implementation subtasks ordered by functional dependencies.

### All Task 6 Documents

| File | Purpose |
|------|---------|
| `plans/dflash-solutions/task6-part1-verification.md` | DFlash config, n_rs_seq chain, opt-in boundary |
| `plans/dflash-solutions/task6-part2-verification.md` | R/S state, cell_copy, backup cells |
| `plans/dflash-solutions/task6-part3-verification.md` | Verification lifecycle, replay integration, tape capture |
| `plans/dflash-solutions/task6-part4-verification.md` | Fallback behavior, corrected VRAM, performance |
| `plans/dflash-solutions/task6-implementation-blueprint.md` | Complete implementation plan (7 sections, 670 lines) |
...
| File | Purpose |
|------|---------|
| `plans/dflash-vram-investigation-context.md` | Original problem statement |
| `plans/dflash-comparison/` (3 files) | Side-by-side old vs current DFlash |
| `plans/dflash-solutions/` (4 files) | Solutions 1-2 feasibility |
| `plans/dflash-solutions/task3-*.md` (5 files) | Task 3 hybrid approach |
| `plans/dflash-solutions/task4-*.md` (6 files) | Task 4 backup/restore feasibility |
| `plans/dflash-solutions/task5-*.md` (8 files) | Task 5 minimal replay extraction |
| `plans/dflash-solutions/task6-*.md` (5 files) | Task 6 implementation plan |
| `plans/dflash-solutions/research-summary.md` | This file |

---

## What Comes Next

All six research phases are complete. The implementation blueprint at `task6-implementation-blueprint.md` is ready for coding. The user may proceed to implementation or provide additional research tasks.

---

## Research Phase 6R: Revised Investigation — IN PROGRESS

**Started:** 2026-08-08
**Tracking:** `plans/dflash-solutions/task6-revised-investigation.md`

### Why This Revision

The previous Task 6 blueprint was based too heavily on Task 5's "minimal replay" proposal, treating the k/v/g/b tape as THE representation rather than comparing it against the proven old v0.3.2 implementation.

Task 6.4 discovered the tape is ~6.5 GiB (not ~24 MB as Task 5 estimated) because v/g/b tensors have `S_v=786,432` dimensions. The previous conclusion was "CPU tape is the only viable strategy."

**The correct question is not "where do we put 6.5 GiB?" but "why does this design need 6.5 GiB when old BeeLlama achieved the same goal with ~200 MB?"**

### New Evidence

- **Old v0.3.2 runtime log:** Shows `dflash gpu ring: allocated 5 layers x 1024 slots x 5120 embd + staging (~200 MB)` — actual working implementation.
- **Old v0.3.2 source:** `dflash_tape_layer` struct stores rank-factored GDN intermediates (k, v, gate, beta, qkv_mixed), NOT full S-state.

### Subtask Progress

| ID | Status | Key Finding |
|----|--------|-------------|
| 6R.1 | COMPLETE | GPU cross ring is ~200 MB (5 layers × 1024 slots × 5120 embd). This is ONLY for hidden-state capture. The tape is a SEPARATE allocation. |
| 6R.2 | COMPLETE | **CRITICAL FINDING:** Old tape stores rank-factored GDN intermediates, not full S-state. Actual tape dimensions: k=[256,1,25], v=[256,8,25], gate=[1,8,25], beta=[1,8,25], qkv=[12768,25]. Total per layer: ~377K elements = ~1.44 MB F32. **GPU tape size: ~70 MB** for 48 layers, 1 slot, 25 max tokens. **Total DFlash GPU memory: ~270 MB** (200 MB ring + 70 MB tape). |
| 6R.3 | COMPLETE | (Produced by 6R.2) Replay/rollback mechanics documented. |
| 6R.4 | COMPLETE | Task 5 assumed full S-state tensors. Old captures rank-factored intermediates. Current upstream CAN expose the same compact intermediates (~134-186 MB tape). |
| 6R.5 | COMPLETE | GDN computation is IDENTICAL between old and current. Same CUDA kernel, same rank-1 update math. Only the tape capture mechanism (~200 lines) needs reinstalling. |
| 6R.6 | COMPLETE | Revised blueprint: ~546 lines (vs ~900 previous), GPU-native tape (~134-186 MB), ~2.8 ms/cycle overhead (vs ~26 ms CPU tape), ~3.3 GB VRAM saved. 6 implementation subtasks defined. |

### Root Cause of 6.5 GiB Discrepancy (Preliminary)

Task 5's estimate assumed the tape captures `k_in`, `v_in`, `g_in`, `b_in` tensors from the current upstream graph — these have dimensions matching the full S-state (`n_embd_s = 786,432`).

The old v0.3.2 implementation captures the GDN intermediates at a DIFFERENT point in the computation — the rank-factored k, v, gate, beta BEFORE they are expanded into the full S-state. The old tape tensors are named `k_conv_predelta`, `v_conv_predelta`, `gate`, `beta` — these are the compact rank-factored components.

**Task 5 was capturing the WRONG tensors.** The correct tape captures the rank-factored intermediates (~70 MB), not the post-expansion tensors (~6.5 GiB).

### All Task 6R Documents

| File | Purpose |
|------|---------|
| `plans/dflash-solutions/task6-revised-investigation.md` | Task 6R tracking document |
