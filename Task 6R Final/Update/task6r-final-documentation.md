...
**Date:** 2026-08-11
**Status:** All 9 development stages + Task 6R completion fixes complete. Build successful.
**Author:** Roo (Architect/Code modes)
**Last Updated:** Task 6R final documentation update — P0/P1/P2 fixes incorporated.
...
8. [Completion Notes](#8-completion-notes)
...
- **CUDA-native conv rebuild:** Convolution state rebuild uses GPU-native CUDA kernel to eliminate PCIe transfers. CPU fallback preserved for non-CUDA backends.
...
- **Fix:** Added conv rebuild step (step 7) to `dflash_custom_replay()` at [`server-dflash-custom.cpp:616-824`](common/server-dflash-custom.cpp:616)
- **Algorithm:** Sliding window shift — positions within old window kept; positions beyond filled from tape qkv data
- **Implementation:** CUDA-native kernel (`ggml/src/ggml-cuda/dflash-custom-conv.cu`) eliminates PCIe transfers. CPU fallback preserved for non-CUDA backends. Partial corruption fix via `cuda_rebuilt_layers` tracking ensures layers already rebuilt by CUDA are skipped if CPU fallback activates mid-loop.
...
| [`common/server-dflash-custom.cpp`](common/server-dflash-custom.cpp) | ~854 | Tape alloc/free, backup/restore, replay, cell_copy, CUDA conv rebuild integration |
| [`ggml/src/ggml-cuda/dflash-custom-conv.cu`](ggml/src/ggml-cuda/dflash-custom-conv.cu) | ~175 | CUDA-native conv rebuild kernel (templated + dynamic) |
| [`ggml/src/ggml-cuda/dflash-custom-conv.cuh`](ggml/src/ggml-cuda/dflash-custom-conv.cuh) | ~78 | CUDA conv rebuild header and host-facing API |
...
- **4 new files:** ~1,357 lines
- **14 modified files:** ~150 lines of custom additions
- **Total custom code:** ~1,507 lines
...
[dflash-custom] conv rebuild: CUDA path, 48 layers, conv_window=2, channels=10240, n_accepted=3
...
```
[dflash-custom] replay execute: n_accepted=3, n_cells=4, layers=48
[dflash-custom] conv rebuild: CPU path, 48 layers, conv_window=2, channels=10240, n_accepted=3
[dflash-custom] replay success: n_accepted=3, n_cells=4, S_k=128, S_v=128, H_k=16, H_v=48
```

### 5.4 Checkpoint Fallback Logs with Reason Codes

```
[dflash-custom] replay skipped (R3): n_accepted=8 > tokens_captured=0
SLT_WRN: dflash custom replay failed (reason: replay_failed) - falling back to checkpoint
```

**Fallback reason codes:**
| Reason Code | Meaning |
|-------------|---------|
| `replay_disabled` | `replay_failed` flag is true (3+ consecutive failures) |
| `replay_not_enabled` | `dflash_custom_is_enabled()` returned false |
| `replay_failed` | `dflash_custom_replay()` returned false |
| `replay_exception` | `dflash_custom_replay()` threw an exception |
| `zero_acceptance` | `n_accepted == 0` — nothing to replay (not a failure) |
| `no_custom_mode` | `slot.dflash_custom` is null |>>>>>>> REPLACE
...
### 6.1 Conv Rebuild — CUDA Kernel Implemented

The convolution state rebuild now uses a GPU-native CUDA kernel ([`dflash-custom-conv.cu`](ggml/src/ggml-cuda/dflash-custom-conv.cu)) that operates directly on GPU tensors, eliminating PCIe transfers. The kernel supports:
- Templated kernels for common window sizes (1, 2, 3) with compile-time loop unrolling
- Dynamic kernel for window sizes not covered by templates
- Per-layer CUDA device detection (supports multi-GPU setups where layers span different GPUs)
- CPU fallback for non-CUDA backends or if CUDA path fails mid-loop
- Partial corruption protection via `cuda_rebuilt_layers` tracking

CPU fallback preserves ~24 MB PCIe per replay for non-CUDA or mixed-backend configurations.

### 6.2 Single Sequence

The replay graph assumes `n_seqs = 1` (per-slot serving). Multi-sequence support is not yet implemented. A warning is logged at startup when `--parallel > 1` with custom mode active (`server-context.cpp:1473-1475`).

### 6.6 Zero Acceptance Edge Case — Fixed

When `n_accepted == 0`, replay is now skipped without incrementing `fail_count`. Zero acceptance is correctly treated as "nothing to replay" rather than a replay failure. The checkpoint rollback path is used as expected. See `server-context.cpp:4322-4323` for the fix and `dflash_fallback_reason = "zero_acceptance"` for observability.
...
## 7. Upstream Merge Considerations>>>>>>> REPLACE
...
This section documents all changes made during the Task 6R completion phase (P0/P1/P2 fixes) beyond the original 9 development stages.

### 8.1 P0 — Zero-Acceptance Fix

**File:** `tools/server/server-context.cpp`
**Change:** When `n_accepted == 0`, replay is now skipped without incrementing `fail_count`. Zero acceptance is correctly treated as "nothing to replay" rather than a replay failure.
**Impact:** Prevents false permanent disable after 3 consecutive zero-acceptance cycles.

### 8.2 P1 — CUDA Conv Rebuild Kernel

**Files Created:**
- `ggml/src/ggml-cuda/dflash-custom-conv.cu` (~175 lines) — CUDA kernel implementation
- `ggml/src/ggml-cuda/dflash-custom-conv.cuh` (~78 lines) — Header and host-facing API

**Files Modified:**
- `common/server-dflash-custom.cpp` — Added CUDA kernel call path with `cuda_rebuilt_layers` tracking

**Key Features:**
- Templated kernels for window sizes 1, 2, 3 with compile-time loop unrolling
- Dynamic kernel for other window sizes using 2D grid
- Per-layer CUDA device detection (multi-GPU support)
- CPU fallback for non-CUDA backends
- Partial corruption protection: if CUDA fails mid-loop, CPU fallback skips already-rebuilt layers

### 8.3 P1 — Parallel Warning

**File:** `tools/server/server-context.cpp` (lines 1473-1475)
**Change:** Warning logged at startup when `--parallel > 1` with custom mode active.
**Impact:** Users are warned that replay assumes single sequence per slot; results may be incorrect for multi-sequence workloads.

### 8.4 P2 — Fallback Reason Codes

**File:** `tools/server/server-context.cpp` (lines 4310-4359)
**Change:** Checkpoint fallback log now includes reason code indicating why replay was not used.
**Reason codes:** `replay_disabled`, `replay_not_enabled`, `replay_failed`, `replay_exception`, `zero_acceptance`, `no_custom_mode`
**Impact:** Operators can distinguish between "replay works but checkpoint used this cycle" vs "replay permanently disabled" vs "zero acceptance (not a failure)".

### 8.5 Deliberate Scope Decisions

**Compile-time guards (`#ifdef BEE_DFLASH_CUSTOM`):** Deferred. The runtime flag gating (`--beefix-dflash-custom`) is sufficient for current needs. Adding compile-time guards would require:
- Modifying 11+ upstream files with `#ifdef`/`#endif` blocks
- Adding `BEE_DFLASH_CUSTOM` define to `CMakeLists.txt`
- Risk of build issues on platforms where the flag is not defined
- No runtime benefit — custom code is already gated at runtime

**Marker comments (`// Task 6R`):** Partially implemented. Key modification points have Task 6R comments. Full coverage deferred as future enhancement.

---

## 9. Next Steps

### 9.1 P2 Improvements (Recommended)

| Priority | Task | Files | Effort | Status |
|----------|------|-------|--------|--------|
| P2 | Add `#ifdef BEE_DFLASH_CUSTOM` guards | Multiple header files | ~15 lines | **Deferred** — see §8.5 |
| P2 | Add `// Task 6R` marker comments | All modification points | ~10 lines | Partial |
| P2 | Remove `LLAMA_API` from `dflash_custom_cell_copy()` | Already done | 0 | Done |

### 9.2 P3 Optimizations (Optional)

| Priority | Task | Description | Status |
|----------|------|-------------|--------|
| P3 | CUDA conv rebuild kernel | GPU-native kernel eliminating PCIe transfers | **Completed** |
| P3 | Configuration struct usage | Use `dflash_custom_config` struct for centralized config | Available |
| P3 | Model-specific capture interface | Virtual interface for SSM architecture extension | Future |
...
*End of final documentation.*>>>>>>> REPLACE
