...
static bool process_prompt(
    ...
    //INSERTED!
    // The special layer ID n_layers represents the final verifier hidden state
    // exposed through the embeddings_nextn API.
    const int32_t n_layers = llama_model_n_layer(llama_get_model(ctx));
    const bool extract_nextn = std::find(
        target_layers.begin(), target_layers.end(), n_layers
    ) != target_layers.end();
    ...
    for (int32_t layer : target_layers) {
        ...
        if (layer == n_layers) {
            data = const_cast<float*>(llama_get_embeddings_nextn(ctx));
        } else {
            data = llama_get_hidden_state(ctx, layer);
        }
...
static int run_self_test() {
    ...
    // cycle with zero precision loss.
    {
        ...
        #ifdef _WIN32
            snprintf(test_ckpt_buf, sizeof(test_ckpt_buf), ".\\cr_self_test_ckpt_%d.bin", (int)getpid());
        #else
